...
APPCFG = LoadConfig()
URL = "https://github.com/sbishnoi26"
...
class ChatBot:
    ...
    def clean_references(documents: List) -> str:
        ...
        for doc in documents:
            # Ensure regex match before accessing groups
            match = re.search(r"page_content=(.*?)( metadata=\{.*\})?", doc)

            if not match:
                print(f"Warning: No match found for regex in document:\n{doc}")
                continue  # Skip processing this document

            content, metadata = match.groups()
            
            metadata = metadata.split('=', 1)[1] if metadata else "{}"
            metadata_dict = ast.literal_eval(metadata)

            # Decode newlines and fix formatting
            ...
            pdf_url = f"{server_url}/{os.path.basename(metadata_dict.get('source', 'unknown.pdf'))}"

            # Append cleaned content
            markdown_documents += (
                f"# Retrieved content {counter}:\n{content}\n\n"
                f"Source: {os.path.basename(metadata_dict.get('source', 'unknown.pdf'))} | "
                f"Page number: {metadata_dict.get('page', 'N/A')} | "
                f"[View PDF]({pdf_url})\n\n"
            )
            counter += 1
